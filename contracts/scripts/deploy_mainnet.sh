...
echo "============================================================"
echo "Deploying Budokan to Mainnet"
echo "============================================================"
...
DECLARE_OUTPUT=$(sncast --profile mainnet declare \
...
if echo "$DECLARE_OUTPUT" | grep -qi "class hash:"; then
    ...
    CLASS_HASH_OUTPUT=$(sncast --profile mainnet utils class-hash --contract-name Budokan --package budokan 2>&1)
...
#
DEPLOYMENT_FILE="deployments/budokan_mainnet_$(date +%Y%m%d_%H%M%S).json"
mkdir -p deployments
