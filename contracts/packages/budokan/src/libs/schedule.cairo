...
mod tests {
    ...
    #[test]
    fn is_valid() {
        // Case 1: All valid with registration (ensure registration is at least
        // MIN_REGISTRATION_PERIOD)
    ...
    fn boundary_conditions() {
        // Test near u64::MAX with valid periods
        let reg_start = Bounded::MAX - 100000;
        let reg_end = reg_start + MIN_REGISTRATION_PERIOD.into();
        let game_start = reg_end + 1000;
        let game_end = game_start + MIN_TOURNAMENT_LENGTH.into();
        ...
        assert!(
            max_schedule.is_registration_open(reg_start + 100),
            "Should handle registration near u64::MAX",
        );
        assert!(
            max_schedule.game.is_active(game_start + 100),
    ...
    fn registration_period_validation() {
        ...
        let game_start: u64 = 10000;
        let game_end: u64 = game_start + MIN_TOURNAMENT_LENGTH.into();

        // Case 1: Valid - registration ends after game start (and is long enough)
